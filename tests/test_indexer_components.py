...
from core.indexing.indexer_component import IndexingError as IndexerError
...
class TestDocumentLoader:
    ...
    def test_load_text_file_success(self):
        ...
        with patch('core.indexing.document_loader.TextLoader', return_value=mock_text_loader):
    ...
    def test_load_text_file_error(self):
        ...
        with patch('core.indexing.document_loader.TextLoader') as mock_text_loader_class:
    ...
    def test_load_text_file_with_encoding(self):
        """Test loading a text file with specific encoding."""
        loader = DocumentLoader()
        
        # Create a mock TextLoader
        mock_text_loader = Mock()
        mock_documents = [Document(page_content="Test content with encoding", metadata={})]
        mock_text_loader.load.return_value = mock_documents
        
        with patch('core.indexing.document_loader.TextLoader', return_value=mock_text_loader):
            documents = loader.load_text_file(Path("test.txt"), encoding="utf-8")
            assert len(documents) == 1
            assert documents[0].page_content == "Test content with encoding"
...
class TestTextSplitter:
    ...
    def test_split_documents_success(self):
        ...
        with patch('core.indexing.text_splitter.get_text_splitter', return_value=mock_splitter):
...
class TestIndexer:
    ...
    async def test_index_documents_success(self):
        ...
        with patch('psutil.virtual_memory') as mock_memory:
            ...
            with patch('core.indexing.indexer_component.aget_qdrant_client') as mock_get_client:
                with patch('core.indexing.indexer_component.get_device') as mock_get_device:
                    with patch('core.indexing.indexer_component.get_dense_embedder') as mock_get_embedder:
                        with patch('core.indexing.indexer_component.Indexer._process_batch') as mock_process_batch:
                            mock_client = Mock()
    ...
    async def test_process_batch_create_collection(self):
        ...
        with patch('psutil.virtual_memory') as mock_memory:
            ...
            with patch('core.indexing.indexer_component.QdrantVectorStore.from_documents') as mock_from_documents:
    ...
    async def test_index_documents_with_bm25(self):
        ...
        config = Config()
        config.index_dense = False
        config.index_bm25 = True
        config.index_hybrid = False
        config.collection_name = "test-collection"
        config.device = "cpu"
        config.indexing_batch_size = 2
        config.memory_threshold = 500 * 1024 * 1024
    ...
    async def test_index_documents_memory_error(self):
        ...
        config.device = "cpu"
        config.indexing_batch_size = 2
        config.memory_threshold = 2 * 1024 * 1024 * 1024  # 2GB threshold
            ...
            with patch('core.indexing.indexer_component.aget_qdrant_client') as mock_get_client:
                with patch('core.indexing.indexer_component.get_device') as mock_get_device:
                    with patch('core.indexing.indexer_component.get_dense_embedder') as mock_get_embedder:
                        ...
                        indexer = Indexer(config)
                        # Should raise IndexerError due to insufficient memory
                        with pytest.raises(IndexerError):
    ...
    async def test_index_documents_client_error(self):
        ...
        with patch('psutil.virtual_memory') as mock_memory:
            ...
            with patch('core.indexing.indexer_component.aget_qdrant_client', side_effect=Exception("Connection failed")):
                indexer = Indexer(config)
                success, status = await indexer.index_documents(documents)
    ...
    def test_indexer_initialization_parametrized(self, index_dense, index_bm25, index_hybrid, should_succeed):
        ...
        config = Config()
        config.index_dense = index_dense
        config.index_bm25 = index_bm25
        config.index_hybrid = index_hybrid
        config.device = "cpu"
        config.indexing_batch_size = 32
        config.memory_threshold = 500 * 1024 * 1024

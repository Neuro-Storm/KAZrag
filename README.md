...
- `scripts/` — вспомогательные скрипты
  ...
  - `evaluate_embeddings.py` — скрипт для оценки моделей эмбеддингов
  - `run_evaluation.py` — скрипт для запуска оценки моделей эмбеддингов
  - `cleanup_test_collections.py` — скрипт для очистки тестовых коллекций
  - `README_evaluation.md` — документация по оценке моделей эмбеддингов
...
### 5. Установка Docling (для обработки документов)

```bash
pip install docling>=2.55.1 easyocr pillow
```

> **Важно:** Docling требует Python 3.10+. Для OCR используется EasyOCR, который не требует установки системных зависимостей и поддерживает русский и казахский языки из коробки. При первом использовании EasyOCR скачает модели (~500MB). Поддерживаемые форматы: PDF, DOCX, PPTX, XLSX, изображения (JPG/PNG), HTML.
...
3. Поместите файлы различных форматов для обработки в папку `pdfs_to_process/`. Поддерживаются следующие форматы:
   - PDF (обрабатываются с OCR и распознаванием таблиц)
   - DOCX
   - PPTX
   - XLSX
   - Изображения JPG, PNG, TIFF (обрабатываются с OCR)
   - HTML
   - Markdown
...
- **Настройка reranker для улучшения релевантности результатов поиска**
...
- Обработка и разбиение документов на Markdown (Docling с EasyOCR)
- Обработка файлов различных форматов:
  - PDF (с OCR и распознаванием таблиц)
...
- **Кэширование поисковых запросов для ускорения повторных запросов**
- **Reranker для улучшения релевантности результатов поиска** (поддерживает локальные модели в формате GGUF) (поддерживает локальные модели в формате GGUF)
...
- [Использование токена HuggingFace](docs/huggingface_token.md)
